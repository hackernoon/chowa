...
import CommentsPopup from './components/CommentsPopup.js';
...
const App = () => {
  ...
  const [hackernoonTouch, setHackerNoonTouch] = useState(true);
  const [formats, setFormats] = useState({});
  const [showLinkOnShortCut, setShowLinkOnShortCut] = useState(false);
  const [commentRange, setCommentRange] = useState(null);

  //  useEffect(() => {
  //   const toolbarElm = document.getElementById("toolbar");
  //   const quillEl = document.getElementsByClassName("quill")[0];
  //   if(hackernoonTouch){
  //     console.log("addingg...", toolbarElm, quillEl);


  //     // if (toolbarElm && quillEl) {
  //     //   // console.log(toolbarElm, quillEl)
  //     //   quillEl.prepend(toolbarElm);
  //     // }
  //   }else{
      
  
  //     console.log("removinggg...", toolbarElm, quillEl)

  //     // quillEl.parentNode.removeChild(toolbarElm);
  ...
  //     // }
  //   }
  // }, [hackernoonTouch, quillRef.current]);
  ...
  const hackernoonTouchChange = () => {
    setHackerNoonTouch(!hackernoonTouch);
  }

  const modules = useMemo(
    () => ({
      toolbar: !hackernoonTouch,
      clipboard: {
        matchVisual: false,
      },
    }),
  ...
  const editorToolbar = useMemo(() => {
    // if(!hackernoonTouch) return null;
    return (
      <Toolbar
        ...
        hackernoonTouch={hackernoonTouch}
        getLink={() => {
          if (quillRef?.current) {
            const quill = quillRef.current.getEditor();
  ...
  }, [formats, quillRef?.current, showLinkOnShortCut, applyFormat, hackernoonTouch]);
  ...
  return (
    <Layout>
      <h1>The HackerNoon Chowa Editor</h1>
      <div style={{marginBottom: "25px"}}>
        <label>Apply the HackerNoon Sauce: </label>
        <input type="checkbox" checked={hackernoonTouch} onChange={() => hackernoonTouchChange()} />
      </div>
      {/* {!hackernoonTouch && ( */}
      {hackernoonTouch ? editorToolbar : null}

      <div
        style={{height: "75vh", width: "100%", position: "relative", display: "flex", gap: "10px"}}
      >
